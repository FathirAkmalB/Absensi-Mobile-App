  ...
  Future<void> getDataSiswasAttn() async {
    ...
    if (token != null) {
      ...
      for (var data in jsonData['siswas']) {
        ...
        var findIzinExact = findIzin.firstWhere(
              (element) {
            DateTime start = DateTime.parse(element['date_start']);
            DateTime end = element['date_end'] != null
                ? DateTime.parse(element['date_end'])
                : DateTime.now().add(const Duration(days: 1));

            if(element['date_end'] == null){
              return start.isBetween(DateTime.now().subtract(Duration(days: 1)), end) ?? false;
            }

            return DateTime.now().isAfterOrEqualTo(start) && DateTime.now().isBeforeOrEqualTo(end);
          },
          orElse: () => null,
        );
        ...
        if (findIzinExact != null) {
          ...
          siswasDatas.add(SiswaAtn(
              ...
              timeOut: findIzinExact["jam_end"] == "00:00:00" ? "-" : findIzinExact["jam_end"]));
    ...
    siswasDatas.sort((a, b) => a.name!.compareTo(b.name!));

    setState(() {});
  }

  Future<bool> postAdjustment() async {
    var sp = await SharedPreferences.getInstance();
          ...
          child: Column(
            children: [
              ...
              Container(
                ...
                child: state == ClassDetailPageStates.ViewingState
                    ? infoKelasWidget(
                  screenW: screenW,
                  screenH: screenH,
                  username: username,
                        ...
                        SizedBox(
                          ...
                          child: TextField(
                            onTap: () async {
                              ...
                              startMapel.text =
                              "${twoDigit(digit: data.hour)}:${twoDigit(digit: data.minute)}:00";
                            ...
                            Column(
                              ...
                              children: [
                                ...
                                Row(
                                  children: [
                                    Text(
                                      "Status: ${statusAbsenToString(data.statusAbsen)}",
                                      style: GoogleFonts.nunito(
                                          fontWeight: FontWeight.bold,
                                          color: const Color.fromARGB(255, 116, 116, 116)),
                                    ),
        ...
        onPressed: () async {
          ...
          if (state == ClassDetailPageStates.EditingState) {
            ...
            const snackBar = SnackBar(
                content: Text("Please Tap the button again to Save it!"));
          ...
          if (state == ClassDetailPageStates.ConfirmationState) {
            ...
            bool isSuccess = await postAdjustment();
...
Widget statusIzinWidget(
    {required screenW, required screenH, required StatusAbsen statusAbsen}) {
  ...
  double font14 = screenW * 0.036;

  return Container(
    padding: const EdgeInsetsDirectional.symmetric(horizontal: 16, vertical: 4),
    decoration: BoxDecoration(
        color: bgColor, borderRadius: BorderRadiusDirectional.circular(4)),
    child: Text(
      text,
      style: GoogleFonts.mulish(
        color: textColor,
...
Widget infoKelasWidget(
    {required screenW, required screenH, required username}) {
  return Container(
